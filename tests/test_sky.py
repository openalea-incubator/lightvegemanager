from lightvegemanager.sky import (
    RATPsky,
    caribuformat_to_ratpformat,
    ratpformat_to_caribuformat,
    discrete_sky,
    CARIBUsky,
)
import os
...
azmoy_file = [0.0, 1.5707964, 3.1415927, 4.712389, 0.0]
ndir_list = 20
hmoy_list = [
    0.15707964,
    0.4712389,
...
@pytest.mark.parametrize(
    "test_input, expected_1, expected_2, expected_3",
    [
...
def test_RATPsky(test_input, expected_1, expected_2, expected_3):
    ...
    for x, y in zip(sky.hmoy, expected_2):
        assert pytest.approx(x) == y
    for x, y in zip(sky.azmoy, expected_3):
        assert pytest.approx(x) == y


def test_ratpformat_to_caribuformat():
    sky = ratpformat_to_caribuformat([0.0, 180.0, 0.0, 180.0], [45.0, 45.0, 90.0, 90.0], [0.0] * 4, False)
    root2_2 = 2**0.5 / 2
    expected = [[root2_2, 0.0, -root2_2], [-root2_2, 0.0, -root2_2], [0, 0, -1], [0, 0, -1]]
    for t, c in zip(sky, expected):
        for x, y in zip(t[1], c):
            assert pytest.approx(x) == y


def test_caribuformat_to_ratpformat():
    directions = [(0.3, (math.sqrt(2) / 2, 0.0, -math.sqrt(2) / 2)), (0.7, (0.0, math.sqrt(3) / 2, -0.5))]

    expected_pc = [0.3, 0.7]
    expected_az = [0.0, math.pi / 2]
    expected_h = [math.pi / 4, math.pi / 6]

    az, h, pc = caribuformat_to_ratpformat(directions)

    for x, y in zip(pc, expected_pc):
        assert pytest.approx(x) == y

    for x, y in zip(az, expected_az):
        assert pytest.approx(x) == y

    for x, y in zip(h, expected_h):
        assert pytest.approx(x) == y
...
@pytest.mark.parametrize("test_input, expected", [("soc", pc_soc), ("uoc", pc_uoc)])
def test_discrete_sky(test_input, expected):
    ...
    for i, p in enumerate(pc):
        assert pytest.approx(p) == expected[i]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # ([4, 5, "soc"], hmoy_list, azmoy_list), BUG avec Caribu Skytools possible sur Vdir
    ],
)
def test_CARIBUsky(test_input, expected_1, expected_2):
    sky = CARIBUsky(test_input)

    #  euse results from RATP
    sky_correct = ratpformat_to_caribuformat(expected_2, expected_1, [0.0] * len(expected_2))

    for t, c in zip(sky, sky_correct):
        for x, y in zip(t[1], c[1]):
